...
task splitPOD5ByChannel {
    ...
    command <<<
        ...
        time pod5 subset ~{pod5dir} --summary ~{summaryFile} --columns channel --output split_by_channel && \
        tar cf split_by_channel.tar split_by_channel/
    >>>

    output {
        File split_by_channel = "split_by_channel.tar"
    }
